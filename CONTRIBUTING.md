...
A read-only client for Ptitchef, with no API key and no account. It reads,
...
──────────────────────────────────────────────────────────  the seam
src/ptitchef/*.ts  http, pacing, storage, reading     ← never imports the SDK
```
...
`src/ptitchef/` file that reaches for the SDK closes that door.
...
Ptitchef is free to read and publishes no crawl delay, which is a reason to
