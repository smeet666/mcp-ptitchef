...
/**
 * Ptitchef publishes no crawl delay in its robots.txt, so this floor is chosen
 * rather than read. It is not negotiable from the outside: configuration can
 * slow the server down, never speed it past a request per second.
 */
...
export const DEFAULT_USER_AGENT = `mcp-ptitchef/${PKG_VERSION} (+${REPO_URL})`;
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  const level = env.PTC_LOG_LEVEL?.trim();
  ...
  if (named && !LOG_LEVELS.includes(named)) {
    process.stderr.write(
      `${PREFIX} error: PTC_LOG_LEVEL="${named}" is not one of ${LOG_LEVELS.join(", ")}; using error.\n`,
    );
  }

  const custom = env.PTC_USER_AGENT?.trim();
  ...
  return {
    ...
    timeoutMs: readInteger(env, "PTC_TIMEOUT_MS", 20_000, 1000, 120_000),
    maxRetries: readInteger(env, "PTC_MAX_RETRIES", 3, 0, 8),
    // A category tree changes on the scale of months, so it is worth holding on
