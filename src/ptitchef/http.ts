...
async function readRefusal(
  ...
  if (PUSH_BACK.has(response.status)) {
    ...
    if (asked !== null && asked > LONGEST_WAIT_MS) {
      return {
        ...
        pushBack: true,
        error: rateLimited(
          `Ptitchef asked this client to wait ${Math.round(asked / 1000)} seconds (HTTP ${response.status}).`,
    ...
    if (attempt >= maxRetries) {
      return {
        kind: "refused",
        pushBack: true,
        error: rateLimited(`Ptitchef asked this client to slow down (HTTP ${response.status}).`, {
          url,
  ...
  if (response.status === 400 || response.status === 422) {
    ...
    return {
      ...
      pushBack: false,
      error: invalidInput(
        "Ptitchef would not accept this request.",
  ...
  if (response.status === 404 || response.status === 410) {
    ...
    return {
      kind: "refused",
      pushBack: false,
      error: notFound("Ptitchef holds nothing at this address.", {
        url,
        status: response.status,
      }),
    };
  ...
  return {
    kind: "refused",
    pushBack: false,
    error: networkError(`Ptitchef answered HTTP ${response.status}.`, {
      url,
      status: response.status,
    }),
  };
...
function readFailure(
  ...
  if (error instanceof Error && error.name === "PtitchefError") {
    throw error;
  }

  if (error instanceof Error && error.name === "AbortError") {
    if (attempt >= Math.min(maxRetries, RETRIES_AFTER_SILENCE)) {
      throw timeoutError(`No answer from Ptitchef within ${timeoutMs}ms.`, { url });
    }
    return error;
  }
  ...
  if (attempt >= maxRetries) {
    throw networkError(`Could not reach Ptitchef: ${failure.message}`, { url });
  }
  return failure;
}
