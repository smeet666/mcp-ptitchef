...
export const SOURCE_NAME = "Ptitchef";
...
export function toToolError(error: unknown): ToolResult {
  const known =
    error instanceof PtitchefError
      ? error
      : new PtitchefError("network_error", error instanceof Error ? error.message : String(error));
...
export const scaledIngredientSchema = z.object({
  original: z.string().describe("The ingredient line exactly as Ptitchef publishes it."),
