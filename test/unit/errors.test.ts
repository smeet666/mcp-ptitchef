...
describe("PtitchefError", () => {
  it("is an Error named PtitchefError", () => {
    const error = new PtitchefError("not_found", "gone");
    expect(error).toBeInstanceOf(Error);
    expect(error).toBeInstanceOf(PtitchefError);
    expect(error.name).toBe("PtitchefError");
    expect(error.message).toBe("gone");
    expect(error.code).toBe("not_found");
  });

  it("keeps the details it is given", () => {
    const error = new PtitchefError("network_error", "boom", {
  ...
  it("attaches a supplied cause to the error itself", () => {
    ...
    const error = new PtitchefError("network_error", "boom", { cause: root });
...
describe("factories", () => {
  ...
  it("each factory keeps the message and the PtitchefError shape", () => {
    for (const error of [
      notFound("m1"),
      invalidInput("m2"),
      rateLimited("m3"),
