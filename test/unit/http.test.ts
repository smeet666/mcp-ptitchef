...
import { fetchPage, parseRetryAfter } from "../../src/ptitchef/http.js";
import { RateLimiter } from "../../src/ptitchef/rateLimiter.js";
import type { Logger } from "../../src/config.js";
import { PtitchefError } from "../../src/errors.js";
...
async function failure(promise: Promise<unknown>): Promise<PtitchefError> {
  const result = await settle(promise);
  if (result.ok) {
    throw new Error(`expected a rejection, resolved with ${String(result.value)}`);
  }
  expect(result.error).toBeInstanceOf(PtitchefError);
  return result.error as PtitchefError;
}
...
describe("fetchPage", () => {
  ...
  it("sends the user agent it was handed, asks for JSON and follows redirects", async () => {
    ...
    expect(call.url).toBe("https://www.ptitchef.com/search");
  ...
  it("turns a thrown transport failure into a PtitchefError", async () => {
    const rec = recorder([() => Promise.reject(new TypeError("socket closed"))]);
    const error = await failure(fetchPage(options(rec.impl, { maxRetries: 0 })));

    expect(error).toBeInstanceOf(PtitchefError);
  });

  it("turns a transport rejection that is not an Error into a PtitchefError", async () => {
  ...
  it("turns a body that cannot be read into a PtitchefError", async () => {
...
describe("the address a page was served from", () => {
  const asked = "https://www.ptitchef.com/recettes/cat/absente";
  ...
  it("is the one the answer came back from, when a redirect moved it", async () => {
    ...
    Object.defineProperty(moved, "url", { value: "https://www.ptitchef.com/recettes" });

    expect((await success(pageFrom(moved))).url).toBe("https://www.ptitchef.com/recettes");
  });
