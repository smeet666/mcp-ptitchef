...
describe("createServer", () => {
  it("announces itself as mcp-ptitchef with the released version", async () => {
    const client = await connected();
    const info = client.getServerVersion();

    expect(info?.name).toBe("mcp-ptitchef");
    expect(info?.version).toBe(packageVersion());
  });

  it("registers list_categories with a description, an output schema and read-only annotations", async () => {
    const client = await connected();
    const { tools } = await client.listTools();
    const tool = tools.find((candidate) => candidate.name === "list_categories");

    if (tool === undefined) {
      throw new Error("list_categories is not registered");
    }
  ...
  it("lists the tools of two separately built servers in the same order", async () => {
    ...
    expect(firstNames).toContain("list_categories");
  });
});
...
describe("INSTRUCTIONS", () => {
  ...
  it("names the tool a caller starts from", () => {
    expect(INSTRUCTIONS).toContain("list_categories");
  });

  it("says a slug is never to be built by hand", () => {
    expect(INSTRUCTIONS).toMatch(/slug/i);
    expect(INSTRUCTIONS).toMatch(/guess|by hand|never build/i);
  });

  it("says the entries shown beside a family are an excerpt", () => {
